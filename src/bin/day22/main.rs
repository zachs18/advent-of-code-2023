...
fn part_1(input: &str) -> usize {
    ...
    // resting_on
    //     .iter()
    //     .inspect(|(rester, restee)| {
    //         eprintln!("{rester} is resting on {restee:?}");
    //     })
    //     .filter(|(_, restees)| restees.len() != 1)
    //     .count()

    // is_rested_on
    //     .iter()
    //     .inspect(|(restee, resters)| {
    //         eprintln!("{restee} is rested on by {resters:?}");
    //     })
    //     .filter(|(_, resters)| resters.len() != 1)
    //     .count()

    fn dbg_output<F, T, R>(mut f: F) -> impl FnMut(&T) -> R
    where
        F: FnMut(&T) -> R,
        T: std::fmt::Debug,
        R: std::fmt::Debug,
